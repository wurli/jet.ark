...
M.help_win = -99
local last_topic --[[@as string?]]

---@param k ark.Kernel
---@param topic string?
M.request_help = function(k, topic)
	local help_comm_id = k:get_comm("positron.help")
	if not help_comm_id then
		return
	end

	topic = topic or last_topic or "help"

	-- This is a 'fire and forget' request. We monitor the comm using
	-- M.listener() and display the result if/when we get a reply
	require("jet.ark.comm.help-backend").show_help_topic(k, help_comm_id, { topic = topic }, function(res)
		if res then
			last_topic = topic
		else
			vim.notify("[jet.ark] Help topic not found: " .. topic, vim.log.levels.WARN)
		end
	end)
end
		...
		vim.schedule(function()
			...
			if vim.api.nvim_win_is_valid(M.help_win) then
				...
				vim.api.nvim_create_autocmd("BufNew", {
					...
					callback = function(e)
						if vim.api.nvim_win_is_valid(M.help_win) and vim.startswith(e.file, "http") then
							show_r_help(e.file, e.buf)
							vim.bo[e.buf].buftype = "nofile"
						end
						if not vim.api.nvim_win_is_valid(M.help_win) then
							return true
						end
					end,
				})
