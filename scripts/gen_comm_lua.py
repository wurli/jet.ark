...
class Emitter:
    ...
    def type_of(self, schema: dict[str, Any], class_hint: str | None = None) -> str:
        ...
        if "oneOf" in schema:
            ...
            for i, member in enumerate(schema["oneOf"]):
                ...
                hint = (
                    f"{class_hint}.{member_name}" if class_hint else member_name
                )
    ...
    def ensure_object_class(self, class_name: str, schema: dict[str, Any]) -> None:
        ...
        for pname, pschema in properties.items():
            ...
            # `<parent>.<field>`
            hint = f"{class_name.split('.', 3)[-1]}.{pname}" if class_name.startswith(self.prefix + ".") else pname
    ...
    def emit_file(self) -> str:
        ...
        parts: list[str] = []
        parts.append(
            "-- This file is auto-generated by scripts/gen_comm_lua.py from "
            f"resources/positron-comms/{self.comm}-{self.side}-openrpc.json."
        )
        parts.append("-- Do not edit by hand.")
        parts.append("")
